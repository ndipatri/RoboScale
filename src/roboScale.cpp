...
void setup();
void loop();
float readScaleState();
void zeroScale();
void calibrateScale();
void scaleInit();
#line 5 "/Users/ndipatri/development/roboGaggia/roboScale/RoboScale/src/roboScale.ino"
...
void loop() {
  ...
  if (!didCalibrate) {
    Serial.print(F("\tPut weight on scale. Press any key to calibrate scale."));
    
    while (Serial.available()) Serial.read(); //Clear anything in RX buffer
    while (Serial.available() == 0) delay(10); //Wait for user to press key

    Serial.print(F("\tBeginning calibration."));
    calibrateScale();
    Serial.print(F("\tDone calibration."));

    didCalibrate = true;
  } else {
    Serial.print(F("\tBeginning to read scale."));
    float currentWeight = readScaleState();
    Serial.print(F("\tDone reading scale."));
...
float readScaleState() {
  if (myScale.available() == true) {
    ...
    return myScale.getWeight(true, 20);
  } else {
    return -1.0;
  }
}
